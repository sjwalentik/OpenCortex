...
public sealed class PostgresUserProviderConfigRepository : IUserProviderConfigRepository
{
    ...
    public async Task<UserProviderConfig?> GetAsync(Guid userId, string providerId, CancellationToken cancellationToken = default)
    {
        ...
        catch (PostgresException ex) when (IsMissingProviderConfigTable(ex))
        {
            return null;
        }
    ...
    public async Task<UserProviderConfig> UpsertAsync(UserProviderConfig config, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw CreateMissingProviderConfigStorageException();
        }
    ...
    private static bool IsMissingProviderConfigTable(PostgresException exception) =>
        string.Equals(exception.SqlState, PostgresErrorCodes.UndefinedTable, StringComparison.Ordinal)
        && exception.MessageText.Contains("user_provider_configs", StringComparison.OrdinalIgnoreCase);

    private static InvalidOperationException CreateMissingProviderConfigStorageException() =>
        new(MissingProviderConfigStorageMessage);
