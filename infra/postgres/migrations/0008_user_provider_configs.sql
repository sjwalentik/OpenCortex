...
CREATE INDEX IF NOT EXISTS idx_user_provider_configs_customer_id ON user_provider_configs(customer_id);

-- Trigger to auto-update updated_at
CREATE OR REPLACE FUNCTION update_user_provider_configs_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = now();
    RETURN NEW;
...
CREATE TRIGGER trg_user_provider_configs_updated_at
    ...
    FOR EACH ROW
    EXECUTE FUNCTION update_user_provider_configs_updated_at();

COMMENT ON TABLE user_provider_configs IS 'User-specific LLM provider configurations with encrypted credentials';
COMMENT ON COLUMN user_provider_configs.encrypted_api_key IS 'API key encrypted with application-level encryption';
COMMENT ON COLUMN user_provider_configs.settings_json IS 'Provider-specific settings like default model, base URL, etc.';
